    ...
    io.sockets.on('connection', function (socket) {
        ...
        socket.on('create_room', function (data) {            
            client.get(socket.id, function (err, socket_info){
                ...
                socket_info_dict.player_state = {'is_playing': 1, 'is_locked': 0, 'playing_song_index':0, 'elapsed': 0, 'timestamp': new Date().getTime(), room_name: data.room_name, playlist: data.playlist};
        ...
        function get_room_info(room_id, callback){
            client.get(room_id, function (err, socket_info){
                callback(null, {room_id: room_id, socket_info: socket_info, users: io.sockets.adapter.rooms[room_id]});
            });
        }
